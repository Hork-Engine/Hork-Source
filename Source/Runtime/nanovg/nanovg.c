...
NVGcontext* nvgCreateInternal(NVGparams* params, FONScontext* fs)
{
...
void nvgReset(NVGcontext* ctx)
{
	...
	state->fontId = FONS_INVALID;
    state->fontUserPtr   = NULL;
}
...
{
	NVGstate* state = nvg__getState(ctx);
    int old = state->compositeOperation;
...
void nvgFill(NVGcontext* ctx)
{
    ...
    if (ctx->params.edgeAntiAlias && state->shapeAntiAlias && state->compositeOperation != NVG_COPY)
...
void nvgFontFaceId(NVGcontext* ctx, int font, void* userPtr)
{
	NVGstate* state = nvg__getState(ctx);
	state->fontId = font;
    state->fontUserPtr = userPtr;
}

void* nvgGetFontFace(NVGcontext* ctx)
{
    NVGstate* state = nvg__getState(ctx);
    return state->fontUserPtr;
}
...
{
    return ctx->params.reallocateTexture(ctx->params.userPtr);
}
...
static void nvg__renderText(NVGcontext* ctx, NVGvertex* verts, int nverts)
{
	...
	// Render triangles.
	paint.image = ctx->params.getFontTexture(ctx->params.userPtr);
              ...
              const char* end)
{
	...
	fonsSetSize(ctx->fs, fontSize*scale);
	fonsSetSpacing(ctx->fs, letterSpacing*scale);
	fonsSetBlur(ctx->fs, fontBlur*scale);
	fonsSetAlign(ctx->fs, textAlign);
    ...
    ctx->params.updateFontTexture(ctx->params.userPtr);
