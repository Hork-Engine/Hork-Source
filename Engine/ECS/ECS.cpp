...
HK_INLINE size_t GetPoolMaxEntities(size_t poolNum)
{
    return (size_t(1) << poolNum) * 1024;
}

HK_INLINE uint32_t GetPoolNum(uint32_t id)
{
    return (id >> 26) & 0xf;
}

HK_INLINE uint32_t GetIndex(uint32_t id)
{
...
uint32_t EntityAllocator::MakeId(uint32_t poolNum, uint32_t index)
{
    HK_ASSERT(index < GetPoolMaxEntities(poolNum));
    HK_ASSERT(poolNum < EntityAllocator::MAX_POOLS);
...
EntityAllocator::~EntityAllocator()
{
    ...
    {
        delete[] m_Pools[i].Entities;
    }
}

void EntityAllocator::AllocatePool(size_t poolNum)
{
    m_Pools[poolNum].Entities = new Entity[GetPoolMaxEntities(poolNum)];
    m_Pools[poolNum].Total = 0;
}

EntityHandle EntityAllocator::EntityAlloc()
{
    uint32_t id;
    uint32_t version;

    {
        ...
        else
        {
            ...
            m_Pools[poolNum].Entities[index].pArchetype = nullptr;
            m_Pools[poolNum].Entities[index].Index = 0;
            m_Pools[poolNum].Entities[index].Version = version = 1;
        }
    }
...
void EntityAllocator::EntityFreeUnlocked(EntityHandle handle)
{
    ...
    e.Index = 0;
    e.Version = Math::Max(1u, e.Version + 1); // Handle wrap
    m_FreeList.Add(id);
}

Entity& EntityAllocator::GetEntityRef(EntityHandle handle)
{
    auto id      = handle.GetID();
    auto poolNum = GetPoolNum(id);
    auto index   = GetIndex(id);
    ...
    Entity& e = m_Pools[poolNum].Entities[index];
    //HK_ASSERT(e.Version == handle.GetVersion());
